...
export default function Section04Portfolio() {
  ...
  return (
    <section className="snap-section bg-[#162840] relative p-0 overflow-hidden">
      ...
      <div className="relative z-10 w-full h-full flex flex-col pt-[5vh] pb-[4vh]">
        <div className="text-center w-full min-h-[5vh] mb-4">
            <span className="font-montserrat text-[13px] font-bold uppercase tracking-widest bg-[#1D3557] px-4 py-1.5 rounded-full border border-[#00B5C5]/30 text-white/90">04 / 09 • Portfólio de Produtos</span>
        </div>
        ...
        <div className="w-full max-w-[1400px] mx-auto flex flex-1 gap-12 px-[5%] mt-auto items-end pb-4 pt-10 relative z-20">
            {/* Mockup Maturidade */}
            <motion.div 
               className="w-1/2 bg-white rounded-xl shadow-[0_15px_30px_-5px_rgba(0,0,0,0.3)] border border-[#1a2535] flex flex-col h-[90%] overflow-hidden"
            ...
            >
               <div className="bg-[#1D3557] px-4 py-3 flex items-center justify-between shrink-0">
                 <span className="font-montserrat text-white font-bold text-[13px] uppercase tracking-wider">Diagnóstico Maturidade</span>
               ...
               <div className="p-6 flex-1 flex flex-col overflow-hidden">
                  <div className="w-full h-8 bg-gray-200 rounded-full mb-6 overflow-hidden flex shrink-0">
                     <div className="h-full bg-[#00B5C5] w-[45%] flex items-center justify-center text-[13px] text-white font-montserrat font-bold tracking-widest">45%</div>
                  </div>
                  <div className="grid grid-cols-1 gap-4 flex-1 overflow-auto custom-scrollbar pr-2">
                     {["Cultura B2B", "Infra de Dados", "Automação", "Processos"].map((lbl, idx) => (
                        <div key={idx} className="flex items-center justify-between font-montserrat text-[13px] text-gray-700 shrink-0">
            ...
            {/* Kanban */}
            <motion.div 
               className="w-1/2 bg-white/95 rounded-xl shadow-[0_15px_30px_-5px_rgba(0,0,0,0.3)] border border-[#1a2535] flex flex-col h-[90%] overflow-hidden backdrop-blur-xl"
            ...
            >
               <div className="bg-[#F5A800] px-4 py-3 flex items-center justify-between shrink-0">
                 <span className="font-montserrat text-[#0D0D0D] font-bold text-[13px] uppercase tracking-wider">Pipeline FollowUp</span>
               ...
               <div className="p-4 flex-1 flex gap-3 overflow-x-auto custom-scrollbar items-start">
                   {["Prospecção", "Negociação", "Fechado"].map((col, cidx) => (
                      <div key={cidx} className="bg-gray-100 flex-1 rounded-lg border border-gray-200 p-3 flex flex-col min-w-[120px] max-h-full overflow-y-auto custom-scrollbar">
                         <span className="font-montserrat text-[12px] font-bold text-gray-800 uppercase mb-3 text-center tracking-widest">{col}</span>
                         {/* Card genérico 1 */}
