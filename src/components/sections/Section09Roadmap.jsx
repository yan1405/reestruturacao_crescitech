...
export default function Section09Roadmap() {
  ...
  // As requested, x:-40 and delayed fade slide per column
  const item = {
  ...
  return (
    <section className="snap-section bg-[#111827] relative p-0 flex flex-col pt-[5vh] pb-[6vh]">
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto h-[20vh] flex flex-col justify-end px-[5%] shrink-0 pb-4">
          <div className="text-xs text-white/40 uppercase tracking-widest font-bold font-montserrat mb-2">09 / 09</div>
          <motion.div initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }}>
            <h2 className="font-impact text-white text-5xl md:text-6xl mb-3 leading-tight tracking-normal">Próximos passos</h2>
            <div className="w-12 h-1 bg-[#00B5C5]" />
          </motion.div>
      </div>
      ...
      <div className="relative z-10 flex-1 w-full max-w-[1400px] mx-auto min-h-[60vh] flex flex-col">
                ...
                <motion.div variants={item} key={idx} className="flex flex-col bg-[#1e293b]/50 border border-white/5 p-8 rounded-xl shadow-2xl h-full backdrop-blur-md relative overflow-hidden shrink-0">
                   ...
                   <div className="flex flex-col gap-8 flex-1">
                     {col.items.map((it, lid) => (
                       <div key={lid} className="flex flex-col w-full relative">
                         <div className="flex items-start justify-between w-full mb-1">
                           <span className="font-montserrat font-bold text-white text-[16px] xl:text-[18px] leading-tight flex-1 pr-2">{it.t}</span>
                           {it.o}
                         </div>
                         <span className="text-[#A8C0D6] text-[14px] xl:text-[15px] font-lato leading-relaxed">{it.b}</span>
          ...
          <div className="w-full mt-auto shrink-0 mb-[2vh]">
            <motion.div className="font-montserrat text-center text-[10px] text-white/30 tracking-[0.3em] font-bold uppercase w-full"
